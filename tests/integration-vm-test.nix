...
#
{ pkgs ? import <nixpkgs> {}, lib ? pkgs.lib, agenix ? null }:
...
let
  ...
  testKimbServicesModule = { config, lib, pkgs, ... }: {
    ...
    in {
      kimb.computed = {
        ...
        in lib.mapAttrs addIP cfg.services;
  ...
  testDesktopConfig = { config, lib, pkgs, ... }: {
    imports = [ 
      testKimbServicesModule 
    ] ++ lib.optional (agenix != null) agenix.nixosModules.default;
  ...
  unitTests = {
    testServiceIPResolution = let
      testConfig = lib.evalModules {
        modules = [ testKimbServicesModule {
          kimb.services = {
            test-service = {
              enable = true;
              port = 8080;
