# Authelia — the a3j.8.2 migration from maitred (host service on historian/dom0).
#
# Same module shape as maitred's, with the storage/users paths identical
# (/var/lib/authelia-main) so the rsync'd state (users_database.yml + db.sqlite3
# + notification.txt) is used verbatim. All five agenix secrets are pre-keyed
# for historian (verified to decrypt with historian's host key); the authelia
# storage-encryption key is the SAME secret on both hosts, so the moved
# db.sqlite3 decrypts.
#
# Gated on historian's OWN registry bucket, so this doesn't touch maitred.
{
  config,
  lib,
  pkgs,
  ...
}: let
  cfg = config.kimb;
in {
  age.secrets = lib.mkIf cfg.services.authelia.enable {
    authelia-jwt-secret = {
      file = ../../secrets/authelia-jwt-secret.age;
      mode = "0400";
      owner = "authelia-main";
      group = "authelia-main";
    };
    authelia-session-secret = {
      file = ../../secrets/authelia-session-secret.age;
      mode = "0400";
      owner = "authelia-main";
      group = "authelia-main";
    };
    authelia-storage-key = {
      file = ../../secrets/authelia-storage-key.age;
      mode = "0400";
      owner = "authelia-main";
      group = "authelia-main";
    };
    authelia-smtp-password = {
      file = ../../secrets/authelia-smtp-password.age;
      mode = "0400";
      owner = "authelia-main";
      group = "authelia-main";
    };
  };

  services.authelia.instances.main = lib.mkIf cfg.services.authelia.enable {
    enable = true;

    settings = {
      server.address = "tcp://0.0.0.0:${toString cfg.services.authelia.port}";
      log = {
        level = "info";
        format = "text";
      };
      totp.issuer = cfg.domain;

      authentication_backend.file.path = "/var/lib/authelia-main/users_database.yml";

      access_control = let
        mkAccessRule = policy: services: {
          domain = map (service: "${service.subdomain}.${cfg.domain}") services;
          inherit policy;
        };
        authServices =
          lib.filterAttrs (
            name: service:
              service.enable
              && service.publicAccess
              && service.auth == "authelia"
          )
          cfg.services;
        oneFactorServices = lib.attrValues (lib.filterAttrs (name: service: name == "grafana") authServices);
        twoFactorServices = lib.attrValues (lib.filterAttrs (name: service: name != "grafana") authServices);
        dynamicRules = lib.flatten [
          (lib.optional (oneFactorServices != []) (mkAccessRule "one_factor" oneFactorServices))
          (lib.optional (twoFactorServices != []) (mkAccessRule "two_factor" twoFactorServices))
        ];
      in {
        default_policy = "deny";
        rules =
          [
            {
              domain = ["auth.${cfg.domain}"];
              policy = "bypass";
            }
            {
              domain = ["vacuum.${cfg.domain}"];
              policy = "one_factor";
            }
          ]
          ++ dynamicRules;
      };

      session = {
        name = "authelia_session";
        inherit (cfg) domain;
        same_site = "lax";
        expiration = "1h";
        inactivity = "5m";
        remember_me_duration = "1M";
      };

      regulation = {
        max_retries = 3;
        find_time = "2m";
        ban_time = "5m";
      };

      storage.local.path = "/var/lib/authelia-main/db.sqlite3";

      notifier = {
        disable_startup_check = false;
        smtp = {
          address = "submissions://smtp.fastmail.com:465";
          username = "kimb@kimb.dev";
          sender = "Authelia <kimb@kimb.dev>";
          identifier = "kimb.dev";
          subject = "[Authelia] {title}";
          timeout = "5s";
          disable_require_tls = false;
        };
      };
    };

    secrets = {
      jwtSecretFile = config.age.secrets.authelia-jwt-secret.path;
      sessionSecretFile = config.age.secrets.authelia-session-secret.path;
      storageEncryptionKeyFile = config.age.secrets.authelia-storage-key.path;
    };

    environmentVariables = {
      AUTHELIA_NOTIFIER_SMTP_PASSWORD_FILE = config.age.secrets.authelia-smtp-password.path;
    };
  };
}
