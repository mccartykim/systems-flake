...
# i3 + emacs writerdeck; libreboot 26.01rev1 firmware. Syncthing for text sync.
{
  ...
  # Remove wizard-generated ~/.config/i3/config if it shadows /etc/i3/config.
  # i3-config-wizard creates this file on first run with a default keybind
  # layout that ignores the NixOS-managed config. The wizard file starts
  ...
  system.activationScripts.creme-i3-cleanup = lib.stringAfter ["users" "etc"] ''
    ...
    if [ -f "$I3CFG" ] && head -3 "$I3CFG" | grep -q "auto-generated by i3-config-wizard"; then
      ...
      mv "$I3CFG" "$I3CFG.wizard-backup"
    fi
    XAUTH="/home/kimb/.Xauthority"
    if [ -f "$XAUTH" ]; then
      chown kimb:users "$XAUTH" 2>/dev/null || true
    fi
  '';
