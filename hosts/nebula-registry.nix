...
let
  ...
  hosts = {
    ...
    mochi = {
      ...
      # Pre-generated STABLE SSH host key (flake_keys/ssh/mochi_host_ed25519_key),
      # baked (passphrase-sealed) into the AVF quick-restore script so mochi's
      # identity survives AVF wipes without the SSH-host-key rotation dance.
      # ROTATED 2026-08-27: the previous key had lived as plaintext in a
      # Bitwarden note (pre-age-sealed-bake era); old key kept as
      # mochi_host_ed25519_key-superseded-2026-08-27. Nebula cert/key .age
      # re-sealed to this key (plain age -r, from flake_keys plaintexts — no
      # CA ceremony); CA itself is fleet-wide, expires 2036.
      publicKey = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPjp7PO7xABI+G8U92oztFyu9CmQemAYAh9q6vsiJdTZ";
      meta = {
